...
export function InventoryEditor({ variantId, quantityAvailable, lowStockThreshold }: { variantId: string; quantityAvailable: number; lowStockThreshold: number }) {
  ...
  return <div className="flex items-center gap-2"><input aria-label="Available quantity" value={quantity} onChange={(e) => setQuantity(e.target.value)} className="w-20 rounded-lg border px-2 py-2"/><input aria-label="Low stock threshold" value={threshold} onChange={(e) => setThreshold(e.target.value)} className="w-20 rounded-lg border px-2 py-2"/><button type="button" onClick={save} className="rounded-full bg-[#0B1D3A] px-3 py-2 text-xs font-bold text-white">Save</button><span className="text-xs text-[#64748B]">{message}</span></div>;
}
