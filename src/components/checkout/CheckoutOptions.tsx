...
function ManualPaymentProofForm({ orderId }: { orderId: string }) {
  ...
  return <div className="mt-4 rounded-2xl border bg-white p-4"><p className="text-sm font-bold text-[#112d33]">Payment proof</p>{submitted ? <p className="mt-2 text-sm text-[#1a7775]">Transaction reference submitted. Support can now verify your payment.</p> : <><label className="mt-3 block text-sm font-semibold">JazzCash transaction ID<input value={reference} onChange={(event) => setReference(event.target.value)} placeholder="e.g. 123456789" className="mt-2 w-full rounded-xl border px-4 py-3 font-normal outline-none focus:border-[#1a7775]" /></label><label className="mt-3 block text-sm font-semibold">Screenshot or receipt <span className="font-normal text-[#718184]">(optional)</span><input type="file" accept="image/jpeg,image/png,image/webp,application/pdf" onChange={(event) => setProof(event.target.files?.[0] ?? null)} className="mt-2 block w-full text-sm font-normal" /></label><button type="button" disabled={loading || reference.trim().length < 3} onClick={submit} className="mt-3 rounded-full bg-[#1a7775] px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50">{loading ? "Submitting..." : "Submit proof"}</button>{error && <p className="mt-2 text-sm text-red-700">{error}</p>}</>}</div>;
}
  ...
  return <div className="grid gap-8 lg:grid-cols-[1fr_.8fr]">
    <section className="rounded-[2rem] border bg-white p-6 shadow-sm sm:p-8">
      <div className="flex items-start justify-between gap-5">
        <div><p className="text-xs font-bold uppercase tracking-[.2em] text-[#1a7775]">Payment details</p><h1 className="display-font mt-2 text-4xl text-[#112d33]">Choose how to pay.</h1></div>
        <WalletCards className="text-[#1a7775]" size={28}/>
      </div>
      <label className="mt-8 block text-sm font-bold text-[#112d33]">Billing country<select value={country} onChange={(event) => { setCountry(event.target.value); if (event.target.value !== "PK") setMethod("paddle"); }} className="mt-2 w-full rounded-xl border bg-white px-4 py-3 font-normal outline-none focus:border-[#1a7775]"><option value="PK">Pakistan</option><option value="AE">United Arab Emirates</option><option value="US">United States</option><option value="OTHER">Other</option></select></label>
      ...
      {method === "jazzcash" && country === "PK" ? <div className="mt-8 rounded-3xl bg-[#f6f7f3] p-5">
        <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-sm text-[#718184]">Send exactly</p><p className="mt-1 text-3xl font-black text-[#112d33]">{amountLabel}</p></div><span className="rounded-full bg-[#f4bf43] px-3 py-1 text-xs font-bold text-[#112d33]">includes USD {TRANSACTION_FEE_USD.toFixed(2)} fee</span></div>
        <div className="mt-5 grid place-items-center rounded-2xl bg-white p-4"><div className="min-h-64 min-w-64 rounded-lg bg-white p-2">{qrDataUrl ? <><span className="sr-only">Payment QR code</span><Image src={qrDataUrl} alt={`JazzCash QR for ${amountLabel}`} width={256} height={256} unoptimized className="h-64 w-64"/></> : <div className="grid h-64 w-64 place-items-center text-center text-sm text-[#718184]">{qrError ? qrError : <Loader2 className="animate-spin"/>}</div>}</div></div>
        <div className="mt-5 rounded-2xl border bg-white p-4"><p className="text-xs font-bold uppercase tracking-widest text-[#718184]">JazzCash wallet</p><p className="mt-2 font-bold text-[#112d33]">{wallet.accountName}</p><div className="mt-2 flex items-center justify-between gap-3"><span className="text-lg font-black tracking-wider text-[#1a7775]">{wallet.number}</span><button type="button" onClick={copyNumber} className="inline-flex items-center gap-2 rounded-full border px-3 py-2 text-xs font-bold">{copied ? <Check size={14}/> : <Clipboard size={14}/>} {copied ? "Copied" : "Copy"}</button></div></div>
        <p className="mt-5 text-sm leading-6 text-[#718184]">After sending the payment, confirm below so we can register your order, then share the transaction screenshot with support. A team member will verify it manually and activate your order.</p>
        {!walletOrderNumber ? <div className="mt-4"><label className="block text-sm font-bold">Email for your order<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" className="mt-2 w-full rounded-xl border bg-white px-4 py-3 font-normal outline-none focus:border-[#1a7775]"/></label><button type="button" disabled={walletOrderLoading || !email} onClick={confirmWalletPayment} className="mt-4 inline-flex items-center gap-2 rounded-full bg-[#112d33] px-5 py-3 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50">{walletOrderLoading && <Loader2 size={16} className="animate-spin"/>} I&apos;ve sent the payment</button>{walletOrderError && <p className="mt-3 text-sm text-red-700">{walletOrderError}</p>}</div> : <div className="mt-4"><p className="rounded-2xl border border-[#1a7775]/30 bg-white px-4 py-3 text-sm font-bold text-[#112d33]"><Check size={14} className="mr-2 inline text-[#1a7775]"/>Order {walletOrderNumber} recorded — now send proof on WhatsApp.</p><a href={whatsappHref} target="_blank" rel="noreferrer" className="mt-4 inline-flex rounded-full bg-[#112d33] px-5 py-3 text-sm font-bold text-white">Send proof on WhatsApp</a></div>}
      </div> : <div className="mt-8 rounded-3xl bg-[#f6f7f3] p-5"><label className="block text-sm font-bold">Email for your order<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" className="mt-2 w-full rounded-xl border bg-white px-4 py-3 font-normal outline-none focus:border-[#1a7775]"/></label><button type="button" disabled={cardLoading || !email} onClick={startCardCheckout} className="mt-5 inline-flex items-center gap-2 rounded-full bg-[#112d33] px-5 py-3 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50">{cardLoading && <Loader2 size={16} className="animate-spin"/>} Continue with card</button>{cardError && <p className="mt-3 text-sm text-red-700">{cardError}</p>}</div>}
      <p className="mt-6 text-xs leading-5 text-[#718184]">Need help? Email <a className="font-bold text-[#1a7775]" href={`mailto:${siteConfig.supportEmail}`}>{siteConfig.supportEmail}</a>.</p>
    </section>
    {walletOrderId && <ManualPaymentProofForm orderId={walletOrderId} />}
    <aside className="h-fit rounded-[2rem] border bg-[#112d33] p-6 text-white sm:p-8"><p className="text-xs font-bold uppercase tracking-[.2em] text-[#f4bf43]">Your order</p><div className="mt-6 grid gap-4">{cart.items.map((item) => <div key={item.id} className="flex justify-between gap-4 text-sm"><span className="text-[#b6cfcb]">{item.productTitle} × {item.quantity}</span><span className="font-bold">{item.unitPrice.currency} {(item.unitPrice.amountMinor * item.quantity / 100).toFixed(2)}</span></div>)}</div><div className="mt-6 border-t border-white/20 pt-5"><div className="flex justify-between text-sm text-[#b6cfcb]"><span>Wallet total</span><span>PKR</span></div><div className="mt-2 text-3xl font-black">{new Intl.NumberFormat("en-PK").format(totalPkr)}</div><p className="mt-3 text-xs leading-5 text-[#b6cfcb]">USD 1 = PKR {exchangeRate.toFixed(2)}. The wallet total includes the flat processing fee.</p></div></aside>
  </div>;
}
