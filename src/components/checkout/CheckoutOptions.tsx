  ...
  return <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_.8fr]">
    <section className="rounded-[2rem] border bg-white p-6 shadow-sm sm:p-8">
      <div className="flex items-start justify-between gap-5">
        <div><p className="text-xs font-bold uppercase tracking-[.2em] text-[#0F766E]">Payment details</p><h1 className="display-font mt-2 text-4xl text-[#0B1D3A]">Choose how to pay.</h1></div>
      ...
      {!SAFEPAY_ENABLED && country !== "PK" ? (
        <div className="mt-8 rounded-3xl bg-[#F1F5F9] p-5 text-center">
          <p className="text-sm font-bold text-[#0B1D3A]">Card checkout is unavailable for this order.</p>
          <p className="mt-2 text-sm leading-6 text-[#64748B]">This checkout uses PKR pricing for card payments and is not available for this order. Please choose JazzCash in Pakistan when available, or email <a className="font-bold text-[#0F766E]" href={`mailto:${siteConfig.supportEmail}`}>{siteConfig.supportEmail}</a> for help.</p>
        </div>
      ) : method === "jazzcash" && country === "PK" ? <div className="mt-8 rounded-3xl bg-[#F1F5F9] p-5">
        <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-sm text-[#64748B]">Send exactly</p><p className="mt-1 text-3xl font-black text-[#0B1D3A]">{amountLabel}</p></div><span className="rounded-full bg-[#0F766E] px-3 py-1 text-xs font-bold text-white">Exact order total</span></div>
        <div className="mt-5 grid place-items-center rounded-2xl bg-white p-4"><div className="grid aspect-square w-full max-w-64 place-items-center rounded-lg bg-white p-2">{qrDataUrl ? <><span className="sr-only">Payment QR code</span><Image src={qrDataUrl} alt={`JazzCash QR for ${amountLabel}`} width={256} height={256} unoptimized className="h-auto w-full max-w-64"/></> : <div className="grid aspect-square w-full max-w-64 place-items-center text-center text-sm text-[#64748B]">{qrError ? qrError : <Loader2 className="animate-spin"/>}</div>}</div></div>
