          ...
          <div className="rounded-[2rem] border bg-white p-6 sm:p-8">
            ...
            <div className="mt-4 grid gap-4">
              {order.items.map((item: OrderItem) => {
                ...
                return (
                  <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 border-b border-[#F1F5F9] pb-4 last:border-0 last:pb-0">
                    <div>
                      <p className="font-bold text-[#0B1D3A]">{item.productName}</p>
            ...
            <div className="mt-6 grid gap-2 border-t border-[#F1F5F9] pt-6 text-sm">
              <div className="flex justify-between text-[#64748B]"><span>Subtotal</span><span>{money(order.subtotal)}</span></div>
              {order.discount.amountMinor > 0 && <div className="flex justify-between text-[#64748B]"><span>Discount</span><span>-{money(order.discount)}</span></div>}
              {order.shipping.amountMinor > 0 && <div className="flex justify-between text-[#64748B]"><span>Shipping</span><span>{money(order.shipping)}</span></div>}
              <div className="flex justify-between text-base font-bold text-[#0B1D3A]"><span>Total</span><span>{money(order.total)}</span></div>
            </div>
          </div>
          ...
          <div className="grid gap-4">
            <div className="rounded-2xl border bg-white p-5">
              ...
              <div className="mt-3 grid gap-2 text-sm">
                ...
                {order.trackingNumber && <div className="flex justify-between"><span className="text-[#64748B]">Tracking</span><span className="font-semibold text-[#0B1D3A]">{order.trackingNumber}</span></div>}
              </div>
            </div>
  ...
  return (
    <main className="min-h-screen bg-[#F1F5F9] px-5 py-14 text-[#0B1D3A]">
      <div className="mx-auto max-w-md rounded-[2rem] border bg-white p-10 text-center">
        ...
        <p className="mt-3 text-sm leading-6 text-[#64748B]">We couldn&apos;t find that order on your account.</p>
        <Link href="/account" className="mt-6 inline-flex rounded-full bg-[#0B1D3A] px-5 py-3 text-sm font-bold text-white">
          Back to your orders
        </Link>
      </div>
    </main>
