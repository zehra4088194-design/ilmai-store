...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try { await requireAdmin(); const { id } = await params; return NextResponse.json(await PromotionService.adminUpdateBanner(id, bannerUpdateSchema.parse(await request.json()))); }
