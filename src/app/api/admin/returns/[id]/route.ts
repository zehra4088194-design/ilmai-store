...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await AuditLogService.record({ actorId: admin.userId, actorRole: admin.role, action: `return_request.${body.status}`, entityType: "order_return_request", entityId: id, metadata: { adminNote: body.adminNote } });
