...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try { await requireAdmin(); const input = parseOrThrow(fulfillmentUpdateSchema, await request.json()); return NextResponse.json(await OrderService.updateFulfillment((await params).id, input)); }
