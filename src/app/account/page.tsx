    ...
    <main className="min-h-screen bg-[#f3f6f1] px-5 py-14 text-[#103d42]">
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mt-6 flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-xs font-bold uppercase tracking-[.2em] text-[#14777a]">My account</p>
        ...
        {orders.length === 0 ? (
          <div className="mt-10 rounded-[2rem] border bg-white p-10 text-center">
            ...
            <p className="mt-3 text-sm leading-6 text-[#6b7f82]">Everything you buy will show up here.</p>
            <Link href="/store" className="mt-6 inline-flex rounded-full bg-[#103d42] px-5 py-3 text-sm font-bold text-white">
              Browse the store
            </Link>
          </div>
        ) : (
            ...
            {orders.map((order) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-[#f5bc50]/25 text-[#103d42]">
                    <Package size={20} strokeWidth={1.5} />
                  </span>
                  <div>
                    <p className="font-bold text-[#103d42]">{order.orderNumber}</p>
                ...
                <div className="flex items-center gap-6">
                  ...
                  <span className="font-bold text-[#14777a]">{money(order.total)}</span>
                  <ChevronRight size={18} className="text-[#789094]" />
                </div>
              </Link>
            ))}
          </div>
