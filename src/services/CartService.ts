...
async function load(cartId: string, db: any) {
  const { data, error } = await db.from("carts").select("*, cart_items(*, product_variants(*, products(title, slug, product_type, delivery_fee_minor)))").eq("id", cartId).single();
