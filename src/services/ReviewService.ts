...
export const ReviewService = {
  ...
  // Same "verified buyer" check submit() enforces server-side, exposed so
  // the review form can gate itself up front instead of letting a guest or
  // a non-buyer fill it out only to have POST /api/reviews reject it after.
  async hasPurchased(userId: string, productId: string): Promise<boolean> { const { data } = await createSupabaseAdminClient().from("order_items").select("id,orders!inner(user_id,payment_status)").eq("product_id", productId).eq("orders.user_id", userId).eq("orders.payment_status", "paid").limit(1).maybeSingle(); return Boolean(data?.id); },
