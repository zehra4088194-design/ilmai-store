...
export const OrderService = {
  ...
  async createFromCart(input: z.infer<typeof checkoutSchema>, options: { adReferral?: string; idempotencyKey?: string; paymentMethod?: "jazzcash" | "safepay" } = {}): Promise<Order> {
    ...
    const shipping = computeShippingMinor(cart.items, input.shippingAddress?.city);
    const totalMinor = Math.max(0, cart.subtotal.amountMinor - discount + shipping + cardFeeMinor);
    const orderNumber = `IL-${new Date().getFullYear()}-${randomInt(100000, 999999)}`;
    const { data: row, error } = await db.from("orders").insert({ order_number: orderNumber, user_id: userId ?? null, status: "pending", payment_status: "pending", fulfillment_status: "unfulfilled", subtotal_minor: cart.subtotal.amountMinor, discount_minor: discount, shipping_minor: shipping, tax_minor: 0, total_minor: totalMinor, currency: cart.subtotal.currency, coupon_code: input.couponCode?.toUpperCase() ?? null, customer_email: input.customerEmail, customer_phone: input.customerPhone ?? null, customer_note: input.customerNote, checkout_idempotency_key: options.idempotencyKey ?? null, metadata: { ...(options.adReferral ? { ad_referral: options.adReferral } : {}), ...(cardFeeMinor > 0 ? { card_processing_fee_minor: cardFeeMinor } : {}) } }).select("id").single();
